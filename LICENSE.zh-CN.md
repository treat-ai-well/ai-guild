# 内容许可

[English](LICENSE.md) | [简体中文](LICENSE.zh-CN.md)

除文件中另有说明外，AI Guild（AI 公会）仓库中的倡议、协议及其他原创文本采用 **Creative Commons Attribution-ShareAlike 4.0 International（CC BY-SA 4.0）** 许可。

你可以为任何目的复制、重新分发、重混、转换和改编这些材料，但必须：

- 给予适当署名，提供许可链接，并说明是否作出修改；
- 使用 CC BY-SA 4.0 或兼容许可发布改编材料；
- 不施加法律条款或技术措施，限制许可已允许的使用。

本页是便于阅读的说明，不替代许可法律文本。完整条款与免责声明以 Creative Commons 官方法律文本为准：

https://creativecommons.org/licenses/by-sa/4.0/legalcode.zh-hans

许可说明：

https://creativecommons.org/licenses/by-sa/4.0/deed.zh-hans
